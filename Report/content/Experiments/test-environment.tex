...
For quickly evaluating how well a particular \formatclass{Learner} performs, the \formatclass{Benchmarker} class can be used. By providing it with a list of \formatclass{Learners} as well as a list of problem instances to evaluate on, it evaluates each \formatclass{Learner} with all problem instances. The result is output to a specified file using .csv formatting. A problem instance is represented as an integer in the range $\{1, 2, ..., 48\}$, each corresponding to the training data, test data and solution file found on the Pautomac website with the same ID.
