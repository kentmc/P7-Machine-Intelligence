...
Initially, a graph representation $G$ of a single state \gls{hmm} is created. The single node has an initial probability of 1, loops to itself with probability 1, and its emission probabilities for each of the $s$ symbols are chosen randomly and normalised.
...
The value $\alpha$ determines how many times the algorithm will try to expand the model. If the $\alpha$ attempts cannot improve the model further, it stops and returns the model. By observing the algorithm, it is clear that most small models improve with the very first extension attempt, while larger models need more attempts to find a new state that improves the model. It is difficult to determine exactly how many extension attempts can be deemed sufficient, as the amount of random extension possibilities is very large. However, as we shall see later in Chapter \ref{chap:experiments} where 100 runs were used, only a handful of experiments ended because of failing to improve after 100 extend attempts.
...
An experiment has thus been conducted, using different $\beta$ values on data set $1$ from the Pautomac competition. The results can be seen in figure \ref{fig:ge-different-thresholds-tested}. Each line represents the mean value of 5 runs of the Greedy Extend algorithm with the specified number of iterations. Some of the plots have been cut off at a point where one of the runs did not manage to extend beyond a certain number of states.
