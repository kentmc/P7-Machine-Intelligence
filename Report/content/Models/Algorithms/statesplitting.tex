...
As the worst performing node is deemed to perform worse because of being involved in the generation of many of the signal in the validation set $V$. As such, it seems meaningful to split the node into more two to split the extensive workload and increase performance.

The \emph{Viterbi heuristic} can be straightforwardly extended to identify more than just one state to split as such producing a set of the worst performing nodes $\mathcal{W}$. The set can be constructed iteratively starting with $\mathcal{W} = \emptyset$ as:
...
More improvements to the \emph{Viterbi heuristic} were considered, mainly including the \emph{n-step Viterbi heuristic} that should have computed the score not only on the output probability in the given significant position, but also the probability of correctly outputting the next $n -1$ symbols of the given signal starting in the explored state to further increase precision. The above described version of the \emph{Viterbi heuristic} would be considered \emph{1-step Viterbi heuristic} in this context. This approach however remains untested due to preference of the \emph{Gamma heuristic} and can be considered for future work.
